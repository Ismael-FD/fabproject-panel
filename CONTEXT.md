...
- ✅ **Context API (sincronización en tiempo real)**: se creó `RestauranteContext` + `Providers` para mantener el estado del restaurante (nombre, bot, email, etc.) sincronizado en toda la app. Al guardar cambios en Configuración General o IA, el sidebar se actualiza de inmediato sin refrescar la página.
- ✅ **Login sin fetch automático**: se eliminó cualquier request a `/restaurantes/mi-restaurante` desde `/login`. Ahora la página de login solo maneja el formulario y el POST a `/auth/login`, evitando el ciclo 401/redirect infinito.
- ✅ **Bot en el sidebar**: el sidebar ahora muestra el nombre del bot configurado (`nombre_bot`) y su estado de conexión con un icono de robot.
- ✅ **Dashboard usa endpoint correcto**: ahora las métricas se cargan desde `GET /pedidos/metricas` (antes `/metricas/dashboard`). Si la API no responde (404/500), el dashboard muestra un mensaje claro indicando la URL que se intentó y sugiere revisar el `NEXT_PUBLIC_API_URL` y que el backend esté corriendo.
- ✅ **API Base URL fallback**: `src/lib/api.js` ahora usa `process.env.NEXT_PUBLIC_API_URL` cuando esté disponible, y si no está configurada en producción falla rápido (para forzar configuración). En desarrollo seguirá usando un ngrok de fallback.
- ℹ️ **Nota importante**: el backend debe estar en funcionamiento y accesible desde la URL configurada. Si la URL del ngrok cambia (por ejemplo, al volver a abrir el túnel), actualizá `NEXT_PUBLIC_API_URL` en `.env` y reiniciá el frontend.
